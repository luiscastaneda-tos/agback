...
| --- | --- |
| Missing expiry/supersession event producers | `src/approvals/in-memory-approval.store.ts` mutates these statuses without publishing; `src/tools/tool-invoker.ts` replaces mismatched requests without emitting superseded. Searching `src/` finds `approval.expired` and `approval.superseded` only in event types. D-023 requires actual emissions. Requested/approved/rejected producers do exist in TaskService/ApprovalDecisionService; do not mark the full lifecycle complete. |
